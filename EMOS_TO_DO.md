...
##TO DOs
* KinEMOS coach-config pass (filed 09/09/2026, do after the lift families of `docs/KINEMOS_P9_PLAN.md`). About twenty `COACH-CONFIG` markers in `src/kinemos` guard judgement calls a coach may move: lift definitions (phase thresholds in `engine/phases.ts` — lift-off 0,1 m/s held 0,05 s, transition rise 15 cm; rep cut and the 2 m/s drop rule in `engine/reps.ts`; the 6 Hz Butterworth cutoff in `engine/signal.ts`; the 45 cm plate default in `engine/calibration.ts`), coaching cues (live-mode cutoff 10 %, the assumed 1,5 m/s snatch threshold on the LV panel, the per-metric `significant` margins), and workflow switches (analyse on import, analyse on the athlete's phone, the 300 MB import cap, the viewer depth presets). The pass: (1) decide which have settled — the activity-scan thresholds and the timing-repair bound stay constants while still measured; (2) one typed settings object per coach, owner-keyed, current constants as defaults, engine reads it as parameters it already takes; (3) one dense KinEMOS row on the settings page; (4) a "reanalyse with current settings" action rather than silent drift of stored reps. Do NOT expose tracker internals (search radius, template blending, confidence floors) — a coach configures what a lift is and what they want told, never how pixels are read. Ship the cutoff together with the raw-vs-smoothed toggle design §6.3 asked for; consider phase thresholds per exercise rather than per coach.
* In the calendar, events that stretch over mulitple days should also stretch here. I wonder if there is a google calendar integration or similar, that would allow for us to also share the calendar as well as have some of these basic functionalities? maybe also have a feed as an input (eg. the national team has a calendar you can subscribe to)
