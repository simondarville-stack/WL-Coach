# EMOS — PROMPT 2: /SYSTEM STYLE GUIDE PAGE

This is step 2 of 3 in rolling out the EMOS design system. It creates
a hidden dev-only route at `/system` that renders every design token
and primitive with live examples. This page is the visual validation
surface — if a component looks wrong here, we fix it here before
touching real pages.

Prerequisites (from Prompt 1 — already committed):
- `src/styles/tokens.css` exists with all CSS variables
- `tailwind.config.js` extends theme with EMOS tokens
- IBM Plex Sans and IBM Plex Mono fonts are loaded
...
once at the end with message: `feat: add /system design system style guide page`.

**IMPORTANT:** This prompt does NOT modify any existing components.
We're only adding a new page and route. Nothing else changes.

---

## STEP 1: CREATE THE PAGE COMPONENT

Create `src/components/system/SystemGuide.tsx`. This is a long file —
each section demonstrates a different token group. Build it
incrementally; run the build after each section.

### 1.1 Skeleton and sidebar nav
...
export function SystemGuide() {
  ...
  return (
    <div className="flex h-full overflow-hidden" style={{ background: 'var(--color-bg-page)' }}>
      ...
      >
        <div
          style={{ padding: '16px 16px 12px', borderBottom: '0.5px solid var(--color-border-tertiary)' }}
        >
      ...
      <div className="flex-1 overflow-y-auto">
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '32px 48px' }}>
          {/* Each section renders as a large scroll-target block */}
          <Foundations />
          <Colors />
          <Typography />
          <SpacingAndBorders />
...
/* Shared spec card — shows a rendered example + its token name */
function Spec({ name, children, value }: { name: string; children: React.ReactNode; value?: string }) {
  return (
    <div style={{
      ...
      <div style={{ minHeight: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 'var(--space-sm)' }}>
        {children}
      </div>
...
Now add each section as a component below `SystemGuide`.

### 1.2 Foundations section
...
function Colors() {
  return (
    <Section id="colors" title="Colors" description="Neutrals for surfaces and text, user-defined accent, four semantic states, nine-ramp entity palette.">
...
### 1.4 Typography section

```tsx
function Typography() {
  return (
    <Section id="typography" title="Typography" description="Two families, two weights, five sizes.">
...
function SpacingAndBorders() {
  return (
    <Section id="spacing" title="Spacing & borders" description="Six spacing stops. Four radius stops. Hairline borders.">
...
function Buttons() {
  ...
  return (
    <Section id="buttons" title="Buttons" description="Three variants × three sizes. Primary is the user's accent color.">
...
function Inputs() {
  ...
  return (
    <Section id="inputs" title="Inputs" description="Text, number, select, textarea, checkbox. Focus uses accent ring.">
...
function Badges() {
  ...
  return (
    <Section id="badges" title="Badges" description="Small semantic markers. Four semantic variants plus neutral.">
...
function ColorDots() {
  return (
    <Section id="dots" title="Color dots" description="Dots encode entity identity — the color of the exercise, category, or athlete itself.">
      <Subhead>Sizes</Subhead>
      <div style={{ display: 'flex', gap: 'var(--space-lg)', alignItems: 'center', marginBottom: 'var(--space-xl)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-xs)' }}>
          <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'var(--color-blue-400)' }} />
          <span style={{ fontSize: 'var(--text-caption)', color: 'var(--color-text-secondary)' }}>6px (inline)</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-xs)' }}>
          <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--color-teal-400)' }} />
          <span style={{ fontSize: 'var(--text-caption)', color: 'var(--color-text-secondary)' }}>8px (standalone)</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-xs)' }}>
          <span style={{ width: '10px', height: '10px', borderRadius: '50%', background: 'var(--color-coral-400)' }} />
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', padding: 'var(--space-md)', background: 'var(--color-bg-secondary)', borderRadius: 'var(--radius-md)' }}>
        {[
          ...
          <div key={ex.code} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)', padding: '6px 10px', fontSize: 'var(--text-label)', background: 'var(--color-bg-primary)', borderRadius: 'var(--radius-sm)' }}>
            <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: ex.dot, flexShrink: 0 }} />
...
function Ribbons() {
  return (
    <Section id="ribbons" title="Ribbons" description="Ribbons encode phase, state, or structural context — where an entity sits within a larger structure.">
...
function StatCards() {
  return (
    <Section id="stat-cards" title="Stat cards" description="Tinted-fill tiles for summary numbers. No border — the grid gap separates them.">
...
function DataTables() {
  return (
    <Section id="data-tables" title="Data tables" description="Mono cells, right-aligned numbers, hairline row dividers, current-row highlight.">
      <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--font-mono)', fontSize: 'var(--text-label)' }}>
        <thead>
          <tr>
            <th style={{ textAlign: 'left', fontFamily: 'var(--font-sans)', fontWeight: 400, fontSize: 'var(--text-label)', color: 'var(--color-text-secondary)', padding: '10px 12px 8px', borderBottom: '0.5px solid var(--color-border-secondary)' }}>Wk</th>
            <th style={{ textAlign: 'left', fontFamily: 'var(--font-sans)', fontWeight: 400, fontSize: 'var(--text-label)', color: 'var(--color-text-secondary)', padding: '10px 12px 8px', borderBottom: '0.5px solid var(--color-border-secondary)' }}>Type</th>
            <th style={{ textAlign: 'left', fontFamily: 'var(--font-sans)', fontWeight: 400, fontSize: 'var(--text-label)', color: 'var(--color-text-secondary)', padding: '10px 12px 8px', borderBottom: '0.5px solid var(--color-border-secondary)' }}>Date</th>
            <th style={{ textAlign: 'right', fontFamily: 'var(--font-sans)', fontWeight: 400, fontSize: 'var(--text-label)', color: 'var(--color-text-secondary)', padding: '10px 12px 8px', borderBottom: '0.5px solid var(--color-border-secondary)' }}>Reps</th>
            <th style={{ textAlign: 'right', fontFamily: 'var(--font-sans)', fontWeight: 400, fontSize: 'var(--text-label)', color: 'var(--color-text-secondary)', padding: '10px 12px 8px', borderBottom: '0.5px solid var(--color-border-secondary)' }}>Tonnage</th>
            <th style={{ textAlign: 'right', fontFamily: 'var(--font-sans)', fontWeight: 400, fontSize: 'var(--text-label)', color: 'var(--color-text-secondary)', padding: '10px 12px 8px', borderBottom: '0.5px solid var(--color-border-secondary)' }}>Avg load</th>
          </tr>
        </thead>
        <tbody>
          {[
...
function PanelsAndModals() {
  return (
    <Section id="panels-modals" title="Panels & modals" description="Side panel for browsing/viewing. Modal for focused creation and destructive confirmation.">
      ...
      <div style={{ position: 'relative', height: '240px', background: 'rgba(0,0,0,0.15)', borderRadius: 'var(--radius-md)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 'var(--space-sm)', justifyContent: 'flex-end', marginTop: 'var(--space-lg)' }}>
            <button style={{ padding: '6px 14px', fontSize: 'var(--text-label)', border: '0.5px solid var(--color-border-secondary)', borderRadius: 'var(--radius-md)', background: 'var(--color-bg-primary)', color: 'var(--color-text-primary)' }}>Cancel</button>
            <button style={{ padding: '6px 14px', fontSize: 'var(--text-label)', fontWeight: 500, border: 'none', borderRadius: 'var(--radius-md)', background: 'var(--color-accent)', color: 'var(--color-text-on-accent)' }}>Create</button>
          </div>
        </div>
      </div>
    </Section>
...
function PageLayout() {
  return (
    <Section id="page-layout" title="Page layout" description="Three framings: standard content page, dense tool page, full-bleed immersive.">
...
Edit `src/App.tsx` to add the `/system` route.

Find the `Routes` block. Add the new route before the catch-all
`<Route path="*" ... />`:
...
Do NOT add it to the sidebar navigation. This is a dev-only route —
reachable by typing the URL directly, not visible in the nav menu.
...
Fix any TypeScript errors. The most likely issue is missing imports —
make sure `ChevronRight` is imported from `lucide-react` at the top
of `SystemGuide.tsx`.
...
Navigate to `http://localhost:5173/system` (or whatever port Vite
chose). Verify:

1. Left sidebar shows all 13 section links
2. Clicking a section scrolls to it
3. Foundations block shows the principles list
4. Colors section shows all neutral swatches, text swatches,
   border swatches, accent, semantics, and all 9 entity ramps with
   7 stops each
5. Typography shows the 5-size scale and both font families
6. Spacing shows 6 spacing stops visually and 4 radius stops
7. Buttons shows 3 variants × 3 sizes = 12 buttons total
8. Inputs shows text, number, select, textarea, disabled
9. Badges shows semantic badges and percentage pills
10. Color dots shows 3 sizes and an in-context exercise list
11. Ribbons shows phase ribbons and a selected-row example
12. Stat cards shows 4-up grid with tinted fill
13. Data tables shows a weekly targets table with current-row highlight
14. Panels & modals shows both previews side-by-side
15. Page layout shows all three framings
16. Everything uses the token CSS variables — no hardcoded colors
17. No console errors

If any section looks off, fix it in `SystemGuide.tsx` before committing.
...
```bash
git add -A
git commit -m "feat: add /system design system style guide page

- New hidden route /system renders every design token and primitive
- Sidebar nav across 13 sections: foundations, colors, typography,
  spacing, buttons, inputs, badges, dots, ribbons, stat cards,
  data tables, panels/modals, page layout
- Uses only CSS tokens from tokens.css — no hardcoded colors
- Dev-only route, not added to sidebar navigation
- Serves as the visual validation surface for the design system"
```
...
2. ✅ `/system` route loads in the browser
3. ✅ All 13 sections render with live examples
4. ✅ Section nav in left sidebar works (scroll-to-section)
5. ✅ Every color swatch shows its token name
6. ✅ All 9 entity ramps display with 7 stops each
7. ✅ Type scale demonstrates 5 sizes clearly
8. ✅ Buttons, inputs, badges render correctly
9. ✅ Data table has current-row highlight with 2px accent border
10. ✅ Panel and modal previews look correct
11. ✅ Page layout shows all three framings
...
Once this is committed and verified, Prompt 3 will extract the
inline styles from `SystemGuide.tsx` into reusable primitive
components in `src/components/ui/`, so the rest of the app can
use them without duplicating styling.

After Prompt 3, we begin page migration (starting with Exercise
Library since it's the most recent build and has the most
