...
export function useWeekPlans() {
  ...
  /**
   * Sync a group week plan to all active members with the default ('update')
   * semantics. The engine lives in src/lib/groupSyncService.ts — the Sync
   * wizard (GroupSyncModal) calls it directly with per-athlete modes; this
   * wrapper keeps the historical sync-everyone entry point.
   */
  const syncGroupPlanToAthletes = async (groupPlanId: string, groupId: string, weekStart: string): Promise<void> => {
    await syncGroupPlanToAllAthletes(groupPlanId, groupId, weekStart);
  };
