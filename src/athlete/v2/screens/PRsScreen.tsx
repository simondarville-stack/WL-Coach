    ...
    <div className="flex flex-col" style={{ minHeight: 'calc(100vh - 80px)' }}>
      <header className="sticky top-0 z-10 bg-[var(--color-bg-page)] px-4 pt-4 pb-3 border-b border-[color:var(--color-border-tertiary)]">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[11px] text-[color:var(--color-text-secondary)] ml-auto">
            {totalPRs} with PRs
          </span>
        </div>
        <div className="relative">
          <Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[color:var(--color-text-tertiary)]" />
          <input
            type="text"
            value={query}
            onChange={e => setQuery(e.target.value)}
            placeholder="Search exercises"
            className="w-full bg-[var(--color-bg-primary)] border border-[color:var(--color-border-tertiary)] rounded-md pl-8 pr-3 py-2 text-sm text-white placeholder:text-[color:var(--color-text-tertiary)] focus:border-[color:var(--color-accent-hover)] outline-none"
          />
        </div>
      </header>
      ...
      <div className="flex-1 px-3 py-3 pb-24 space-y-3">
        ...
        {rows != null && grouped.length === 0 && !error && (
          <div className="text-center text-xs text-[color:var(--color-text-secondary)] italic py-12">
        ...
        {grouped.map(({ category, rows: catRows }) => {
          ...
          return (
            <section
              key={category}
              className="rounded-xl bg-[var(--color-bg-primary)] border border-[color:var(--color-border-tertiary)] overflow-hidden"
            >
              <button
                type="button"
                onClick={() =>
              ...
              >
                ...
                <span className="text-[11px] uppercase tracking-wider text-[color:var(--color-text-primary)] font-semibold">
                  {category}
                </span>
                <span className="text-[length:var(--text-caption)] text-[color:var(--color-text-secondary)] ml-auto tabular-nums">
                        ...
                        <div className="text-right flex-shrink-0">
                          {row.implied1RM != null ? (
                            <>
                              ...
                              <div className="text-[length:var(--text-micro)] text-[color:var(--color-text-secondary)] uppercase tracking-wider mt-0.5">
                                kg e1RM
                              </div>
                            </>
                          ) : (
                            <span className="text-xs text-[color:var(--color-text-tertiary)] italic">no PR</span>
                          )}
                        </div>
                      </button>
                    </li>
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-[color:var(--color-border-tertiary)]">
          <div className="flex items-center gap-2">
            <Trophy size={14} className="text-[color:var(--color-accent)]" />
            <span className="text-sm font-bold text-white">Log a PR — pick an exercise</span>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="tap-y text-xs text-[color:var(--color-text-secondary)] hover:text-white px-2 py-1"
          >
            Cancel
          </button>
        </div>
        <div className="px-3 py-2 border-b border-[color:var(--color-border-tertiary)]">
          <div className="relative">
            ...
            <input
              ...
              onChange={e => setQuery(e.target.value)}
              placeholder="Search exercises"
              className="w-full bg-[var(--color-bg-page)] border border-[color:var(--color-border-tertiary)] rounded-md pl-8 pr-3 py-2 text-sm text-white placeholder:text-[color:var(--color-text-tertiary)] focus:border-[color:var(--color-accent-hover)] outline-none"
            />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {filtered.length === 0 ? (
            <div className="text-xs text-[color:var(--color-text-secondary)] italic px-4 py-8 text-center">
              No exercises match.
            </div>
          ) : (
            <ul className="divide-y divide-[color:var(--color-border-tertiary)]">
                ...
                <li key={r.exercise.id}>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[length:var(--text-caption)] text-[color:var(--color-text-secondary)]">{r.exercise.category || 'Other'}</div>
                    </div>
                    {r.implied1RM != null && (
                      <span className="text-xs text-[color:var(--color-text-secondary)] tabular-nums flex-shrink-0">
