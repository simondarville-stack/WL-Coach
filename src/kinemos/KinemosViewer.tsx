...
import { gradeAnalysis, type CameraStability } from "./engine/grade";
...
export function KinemosViewer() {
  ...
  const [massSource, setMassSource] = useState<"logged" | "manual" | null>(
    null,
  );
  ...
  const [coachBoundaries, setCoachBoundaries] = useState<
    PhaseBoundary[] | null
  >(null);
  ...
  const metrics = useMemo(
    () => pathMetrics(points, calibration),
    [points, calibration],
  );
  ...
  const proposal = useMemo(
    () => (kinematics ? proposePhases(kinematics) : null),
    [kinematics],
  );
  ...
  const spans = useMemo(
    () => spansFrom(boundaries, DEFAULT_PHASE_SET),
    [boundaries],
  );

  const liftMetrics = useMemo(
    () =>
      kinematics && spans.length > 0
  ...
  const repSummary = useMemo(
    () => (kinematics ? summariseRep(kinematics) : null),
    [kinematics],
  );
  ...
  const grade = useMemo(
    ...
    [kinematics, server, calibration, camera, points.length],
  );
    ...
    const timer = window.setTimeout(() => {
      void (async () => {
        try {
          ...
          setSaveError(
            "Changes could not be saved — they are still on screen, but not stored.",
          );
        }
      })();
    }, SAVE_DEBOUNCE_MS);
    ...
    (index: number, t: number) => {
      ...
      setCoachBoundaries((current) => {
        ...
        const next = base.map((b, i) =>
          i === index ? { ...b, t, source: "coach" as const } : b,
        );
        return next;
      });
    },
  ...
  const deleteMarkHere = useCallback(() => {
    ...
    dirtyRef.current = true;
    setPoints((current) =>
      current.filter((p) => Math.abs(p.t - currentT) > 1e-6),
    );
  }, [currentT]);
  ...
  const clearMarks = useCallback(async () => {
    const ok = await confirmDialog({
      ...
      message:
        "The bar path for this rep will be removed. Nothing else is affected.",
      confirmLabel: "Clear",
      tone: "danger",
    });
    if (!ok) return;
  ...
  const measureValue = useMemo(() => {
    if (tool === "distance" && measurePoints.length === 2) {
      return calibration
        ? formatDistance(
            distanceCm(calibration, measurePoints[0], measurePoints[1]),
            true,
          )
        : formatDistance(
            Math.hypot(
  ...
  const addMeasurePoint = useCallback(
    (point: PxPoint) => {
      setMeasurePoints((current) =>
        current.length >= measureArity ? [point] : [...current, point],
      );
    },
    [measureArity],
  );
  ...
  const takeSnapshot = useCallback(async () => {
    ...
    try {
      ...
      setSaveError(
        "The snapshot could not be stored — check that KinEMOS storage is configured.",
      );
    } finally {
  ...
  if (clipError || !clip) {
    return (
      <div style={{ padding: "var(--space-xl)" }}>
        <ErrorState
          message={clipError ?? "Clip not found."}
          onRetry={() => navigate("/kinemos")}
        />
      </div>
    );
  }
      ...
      <div style={{ flexGrow: 1, display: "flex", minHeight: 0 }}>
          ...
          {TOOLS.map(({ id: toolId, label, icon: Icon, key }) => (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                background:
                  tool === toolId ? "var(--color-accent-muted)" : "transparent",
                color:
                  tool === toolId
                    ? "var(--color-accent)"
        ...
        >
          ...
          {analysable && status === "ready" && server && (
            <>
              ...
              >
                ← → step a frame · ⇧ steps ten · space plays · V/C/M/D/A pick a
                tool · R resets phase edges · shift-drag pans
              </p>
            </>
          )}
        </main>
        ...
        >
          ...
          <ReadoutRail
            ...
            markedHere={
              currentT !== null &&
              points.some((p) => Math.abs(p.t - currentT) < 1e-6)
            }
