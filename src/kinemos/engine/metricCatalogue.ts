...
export const METRIC_CATALOGUE: readonly MetricDefinition[] = [
  ...
  {
    id: 'peakPowerPerKg',
    label: 'Peak power / bodyweight',
    unit: 'W/kg',
    decimals: 1,
    betterWhen: 'higher',
    why: 'Barbell power per kilo of lifter — the power figure that survives a comparison across body sizes. Needs the bar mass and the athlete’s bodyweight.',
    read: l => l.metrics.peakPowerPerKgBwW ?? null,
    significant: 0.5,
  },
  {
    id: 'duration',
...
export function fromStoredMetrics(raw: unknown): StoredMetrics | null {
  ...
  return {
    ...
    peakPowerPerKgBwW: numberOrNull(r.peakPowerPerKgBwW),
    analyzer,
    jerk,
    summary,
  };
