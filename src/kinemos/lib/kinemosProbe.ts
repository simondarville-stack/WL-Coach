 ...
 *
 * Re-exported from the engine rather than defined here: the import probe and
 * the frame server have to agree about what "variable frame rate" means — a
 * clip flagged CFR at import but treated as VFR by the viewer would be graded
 * on one basis and measured on another. The rule (and its rationale) lives in
 * `engine/frameServer.ts` as `detectVfr`; this name is kept for the probe's
 * callers and tests.
 */
import { detectVfr as isVariableFrameRate } from '../engine/frameServer';
