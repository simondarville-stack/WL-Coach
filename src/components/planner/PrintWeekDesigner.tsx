...
interface PrintWeekDesignerProps {
  /** Provide either athlete or group. Athlete enables the optional
   *  "Include athlete's actuals" log overlay; group prints the bare
   *  plan with the group name in the header instead. */
...
export function PrintWeekDesigner({
  ...
  const headerSubParts = athlete
    ? [age !== null && `${age} years`, athlete.bodyweight && `${athlete.bodyweight} kg`, athlete.weight_class]
        .filter(Boolean)
    : group
    ? ['Group plan']
    : [];
              ...
              <header className="dz-header">
                <div className="dz-header-left">
                  ...
                  {opts.showAthleteDetails && headerName && (
                    <>
                      <h1 className="dz-athlete-name">{headerName}</h1>
                      {headerSubParts.length > 0 && (
                        <p className="dz-athlete-sub">{headerSubParts.join(' · ')}</p>
                      )}
                    </>
                  )}
                </div>
