/**
 * ShareGroupModal — host-side UI to invite another coach to co-coach or
 * view a training group. Adapted from ShareAthleteModal.
 *
 * Sharing a group also grants access to its member athletes (the
 * group→athlete cascade in accessScope), and a co_coach can add their
 * own athletes to the group. The modal spells this out so the host
 * understands the blast radius before inviting.
 */
import { useEffect, useState } from 'react';
import { X, UserPlus, Check, Clock, Slash } from 'lucide-react';
import type { TrainingGroup, CoachProfile, TrainingGroupCollaborator, CollaboratorRole } from '../lib/database.types';
...
export function ShareGroupModal({ group, onClose }: Props) {
  ...
  const { listCollaboratorsFor, inviteCoach, revokeAccess } = useTrainingGroupCollaborators();

  const [coaches, setCoaches] = useState<CoachProfile[]>([]);
  const [collaborators, setCollaborators] = useState<TrainingGroupCollaborator[]>([]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-xl w-[480px] max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
          <div>
            <h2 className="text-base font-semibold text-gray-900">Share {group.name}</h2>
        ...
        <div className="px-4 py-3 flex-1 overflow-y-auto space-y-3">
          ...
          <div className="text-[11px] text-blue-800 bg-blue-50 border border-blue-200 rounded px-2.5 py-1.5">
            Sharing this group also grants access to its member athletes. A co-coach
            can add their own athletes into the group, which then become shared with you too.
          </div>
...
function CollaboratorRow({
  ...
  collaborator: TrainingGroupCollaborator;
  coachName: string;
  canManage: boolean;
  onRevoke: () => void;
  busy: boolean;
