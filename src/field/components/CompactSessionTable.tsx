 ...
 *
 * One row per planned exercise: name / heaviest segment in Stacked Load
 * Notation / average load / total reps / total sets — the Max → Ø → R → S
 * order the planner's condensed week cards read in (DayCardCondensed), so
 * the coach scans the same shape on both surfaces. Percentage loads
 * resolved to kilograms render in amber under the stacked segment. Rows at
 * or above the coach's bold threshold render bold.
 */
    ...
    <table className="w-full text-xs" style={{ tableLayout: 'fixed', borderCollapse: 'collapse' }}>
      <thead>
        <tr className="text-[length:var(--text-caption)] uppercase tracking-wide text-[color:var(--color-text-secondary)]">
          ...
          {/* Wide enough that a free-text load ("Moderat") plus its set
              multiplier stays on one line instead of wrapping the stack. */}
          <th className="text-center font-normal py-1 w-24">Top</th>
          <th className="text-right font-normal py-1 w-12">Avg</th>
          <th className="text-right font-normal py-1 w-9">Reps</th>
          <th className="text-right font-normal pr-3 py-1 w-9">Sets</th>
        </tr>
      </thead>
      <tbody>
        {rows.map(r => {
          ...
          return (
            <tr key={r.key} className={`border-t border-gray-800/80 ${tone}`}>
              ...
              <td className="pr-3 py-1.5 text-right tabular-nums">{r.totalSets || '—'}</td>
            </tr>
          );
        })}
      </tbody>
