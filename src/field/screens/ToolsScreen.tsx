...
const VERDICT_STYLES: Record<PrilepinVerdict, { cls: string; label: string }> = {
  optimal: { cls: 'bg-blue-950/60 border-blue-800/60 text-[color:var(--color-accent)]', label: 'Optimal dose' },
...
function NumField({
  ...
  return (
    <label className="flex items-center gap-1.5 flex-1 min-w-0">
      <span className="text-[11px] text-[color:var(--color-text-secondary)] whitespace-nowrap">{label}</span>
      <input
        type="text"
        inputMode="decimal"
        value={value}
        ...
        className="w-full min-w-0 font-mono text-sm bg-[var(--color-bg-page)] border border-[color:var(--color-border-tertiary)] rounded-lg px-2.5 py-1.5 text-right text-white focus:outline-none focus:border-[color:var(--color-border-primary)]"
      />
      {suffix && <span className="text-[11px] text-[color:var(--color-text-secondary)]">{suffix}</span>}
    </label>
  );
}
  ...
  return (
    <div className="max-w-2xl mx-auto px-3 pt-4 pb-4 space-y-3">
      <div className="px-1">
        ...
        <p className="text-xs text-[color:var(--color-text-secondary)]">Rep-max estimator · Prilepin zones</p>
      </div>
      <XrmCard />
      <PrilepinCard />
    </div>
...
function XrmCard() {
  ...
  return (
    <section className="bg-[var(--color-bg-primary)] border border-[color:var(--color-border-tertiary)] rounded-xl px-3 py-3">
      <h2 className="text-[11px] font-semibold uppercase tracking-wide text-[color:var(--color-text-secondary)] mb-2">
        Rep-max estimator
      </h2>
      <div className="flex items-center gap-3">
        <NumField label="Weight" value={weightText} onChange={setWeightText} suffix="kg" />
        <span className="text-[color:var(--color-text-tertiary)] text-xs">×</span>
        <NumField label="Reps" value={repsText} onChange={setRepsText} />
      </div>

      {reps != null && !validReps && (
        <p className="text-[length:var(--text-caption)] text-amber-400 italic mt-2">Reps must be a whole number 1–15.</p>
      )}

      {oneRm != null && (
        <>
          <p className="text-sm text-[color:var(--color-text-primary)] mt-3">
            Estimated 1RM{' '}
            <span className="font-mono font-semibold text-white">{fmt(roundToHalf(oneRm))} kg</span>
            <span className="text-[length:var(--text-caption)] text-[color:var(--color-text-secondary)] ml-1.5">(11-formula average)</span>
          </p>
          <table className="w-full text-xs mt-2">
            ...
            <tbody>
              <tr className="border-t border-gray-800/80 text-[color:var(--color-text-primary)]">
                {REP_RANGE.map(r => (
          ...
          <p className="text-[length:var(--text-caption)] text-[color:var(--color-text-tertiary)] mt-1">Projected load (kg) at each rep count.</p>
        </>
      )}
    </section>
  );
    ...
    <section className="bg-[var(--color-bg-primary)] border border-[color:var(--color-border-tertiary)] rounded-xl px-3 py-3">
      ...
      <div className="flex items-center gap-1.5 text-xs mt-2.5">
        <input
          ...
          placeholder="reps"
          className="w-14 font-mono text-xs bg-[var(--color-bg-page)] border border-[color:var(--color-border-tertiary)] rounded-lg px-2 py-1 text-right text-white focus:outline-none focus:border-[color:var(--color-border-primary)]"
        />
        ...
        />
        {nl != null && (
          <span className="text-[color:var(--color-text-secondary)] ml-1">
            = <span className="font-mono font-medium text-white">{nl}</span> NL
          </span>
        )}
        {!activeZone && (repsText || setsText) && (
          <span className="text-[length:var(--text-caption)] text-[color:var(--color-text-secondary)] italic ml-1">enter intensity to compare</span>
        )}
      </div>
        ...
        <tbody>
          {PRILEPIN_ZONES.map(z => {
            ...
            const rowCls = isActive
              ? 'bg-blue-950/50 border-l-2 border-l-[color:var(--color-accent)] text-white font-medium'
      ...
      {pctText !== '' && !activeZone && (
        <p className="text-[length:var(--text-caption)] text-amber-400 mt-1 italic">
